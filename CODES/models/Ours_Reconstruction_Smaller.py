...
class Model(BaseModel):
    ...
    def _init_create_networks(self):
        self._G = NetworksFactory.get_by_name('Ours_Reconstruction',
    ...
    def forward(self, keep_data_for_visuals=False, isTrain=True):
        ...
        self.last_sharps = blur_list[0].repeat(1,self.inter_num,1,1)
        ...
        VideoWriter = cv_utils.save_videos(os.path.join(self._opt.output_dir, self.dataname + '.mp4'),
                                           self.inter_num, (self._opt.width*2, self._opt.height))
        ...
        for blur_index in range(len(blur_list)):
            ...
            # forward
            self.est_sharps = self._G(self.cur_blur, self.cur_event, self.last_sharps, self.last_blur, self.last_events)

            # update.
            if blur_index % self.sequence_num == 0 and blur_index + 1 < len(blur_list):
                self.last_sharps = blur_list[blur_index+1].repeat(1, self.inter_num, 1, 1)
                self.last_blur = blur_list[blur_index+1]
                self.last_events = self._input_event[:, (blur_index+1)* self.inter_num * self.eventbins_between_frames:
                                                        (blur_index+2) * self.inter_num * self.eventbins_between_frames]
            else:
                self.last_sharps = self.est_sharps.detach()
            ...
            if not os.path.exists(path):
                os.makedirs(path)
            for idxs in range(self.inter_num):
                path_i = os.path.join(path,
                                      str(self.imgcount + idxs) + '_reconstruction.png')
                cv_utils.debug_save_tensor(self.est_sharps[:, idxs:(idxs + 1)], path_i, rela=False, rgb=False)
            self.imgcount = self.imgcount + self.inter_num
            ############# save video #############
            concat = torch.cat((self.cur_blur.repeat(1,self.inter_num,1,1), self.est_sharps), -1)
